...
1. Drop the image next to `design.js`. Any name works, and `.jpg`, `.png`,
   `.webp` and `.avif` are all accepted — the four rendered `.png` outputs are
   ignored, so it won't mistake one of those for the portrait.
2. Re-run `node design.js` — it prints which portrait it used.

If several images are present it takes one named `photo.*` first, otherwise
the first alphabetically, and says which. Set `photo:` in `CONFIG` to pin an
exact file.
